...
cmd:option('-output_predictions','prediction_result/res_predictions_testall.json','path to output prediction captions')
...
cmd:option('-test_img_info_file','data/coco/coco_test_raw.json','path to the json containing the test image information')
cmd:option('-cap_h5_file','data/coco/cocotalk_cap_label.h5','path to the h5 file containing the caption label and infomation')
cmd:option('-kNN_cap_json_file','data/coco/10NN_cap_testall_cider.json','path to the json file containing NN caps for testall')
...
local vocab_info = utils.read_json(opt.img_info_file)
local vocab = vocab_info.ix_to_word
local img_info = utils.read_json(opt.test_img_info_file)
...
local function eval_split(split, evalopt)
  ...
  for ii=sIdx,eIdx do
    ...
    file_path = string.gsub(file_path, 'test2014', 'test2015')
    ...
    local sample_opt = {sample_max = opt.sample_max, beam_size = opt.beam_size, temperature = opt.temperature}
    local seq, probs, beam_seq = protos.lm:sample(emb_feats, sample_opt)

    local sents = net_utils.decode_sequence(vocab, seq)                 -- idx to word
    local entry = {image_id = img_info[ii].id, caption = sents}
    ...
    if ii % 100 == 0 then collectgarbage() end
  end
...
local predictions = eval_split(opt.split)
