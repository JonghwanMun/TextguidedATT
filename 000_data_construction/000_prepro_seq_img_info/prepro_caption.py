"""
This code is written by Andrej karpathy.
This code is downloaded from: https://github.com/karpathy/neuraltalk2

Preprocess a raw json dataset into hdf5/json files for use in loader
...
def prepro_captions(imgs):
  """
  changing word to lower case and deleting punctuation
  """
  ...
  for i,img in enumerate(imgs):
    ...
    for j,s in enumerate(img['captions']):
      txt = str(s).lower().translate(None, string.punctuation).strip().split() # delete punctuation
...
def encode_captions(imgs, params, wtoi):
  ...
  for i,img in enumerate(imgs):
    ...
    for j,s in enumerate(img['final_captions']):  # j is index of captions
...
def main(params):
  ...
  imgs = json.load(open(params['input_json'], 'r'))  # coco_raw_ordered.json
  ...
  for i,img in enumerate(imgs):
    ...
    if 'id' in img: jimg['id'] = img['id'] # copy over & maintain an id, if present (e.g. coco ids, useful)
