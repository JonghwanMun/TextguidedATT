...
-------------------------------------------------------------------------------
local layer, parent = torch.class('nn.sentenceEncoder','nn.Module')
...
function layer:__init(uparams, utables, opt)
  ...
  self:_createInitState(1) -- will be lazily resized later during forward passes
	...
	print('\n----------Sentence Encoder initialized')
	print(string.format('Sentence Encoder backend    : %s (sentence vector size)',self.backend))
	print(string.format('Sentence Encoder rnn size   : %d ',self.rnn_size))
	print(string.format('Sentence Encoder word dim   : %d ',self.word_dim))
	print(string.format('Sentence Encoder seq len    : %d ',self.seq_length))
	print(string.format('Sentence Encoder vocab size : %d ',self.vocab_size))
	print('')
end
...
function layer:createClones()
  ...
  print('constructing clones inside the Sentence Encoder Model')
