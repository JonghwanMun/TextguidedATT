...
local LSTM = require 'layers.LSTM'
...
function layer:__init(opt)
  parent.__init(self)
	...
	self.sharedE = utils.getopt(opt, 'sharedE', true)
	self.backend = utils.getopt(opt, 'backend', 'cudnn')
  ...
  -- options for scheduled shampling
  self.scheduled_sampling= utils.getopt(opt, 'scheduled_sampling', -1)
	self.eta = utils.getopt(opt, 'eta', 1)
  -- create the core lstm network. note +1 for both the START and END tokens
  self.lookup_table = nn.LookupTable(self.vocab_size + 1, self.input_encoding_size)
  if self.sharedE == true then
    self.core = LSTM.lstm(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout, self.backend, self.lookup_table)
  else
    self.core = LSTM.lstm(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout, self.backend, nil)
  end
  self:_createInitState(1) -- will be lazily resized later during forward passes

	print('\n-------------Language Model initialized')
	print(string.format('LM vocab size  : %d',  self.vocab_size))
	print(string.format('LM input dim   : %d',  self.input_encoding_size))
	print(string.format('LM rnn size    : %d',  self.rnn_size))
	print(string.format('LM num layer   : %d',  self.num_layers))
	print(string.format('LM dropout     : %.2f',dropout))
	print(string.format('LM seq length  : %d',  self.seq_length))
	print(string.format('LM sampling    : %d',  self.scheduled_sampling))
	print(string.format('LM eta         : %.2f',self.eta))
	print(string.format('LM backend     : %s',  self.backend))
	print(string.format('LM sharedE     : %s',  self.sharedE))
	print('')
end
...
function layer:_createInitState(batch_size)
  ...
  self.num_state = #self.init_state -- num_layer*2
end
...
function layer:update_eta(eta)
  self.eta = eta
end

function layer:set_vocab(vocab)
  self.vocab = vocab 
end
...
function layer:print_caption(cap)
  for i=1,self.seq_length do
    if cap[i] == self.vocab_size+1 then
      io.write('<E> ')
    elseif cap[i] == 0 then
      io.write('<Z> ')
    else
      io.write(self.vocab[tostring(cap[i])], ' ')
    end
  end
end
  ...
  for t=1,self.seq_length+2 do
    ...
    elseif t == 2 then
      ...
      if sample_max == 1 then
        -- use argmax "sampling"
        sampleLogprobs, it = torch.max(logprobs, 2) -- max from row values
        it = it:view(-1):long()
      else
    ...
    state = {}
    for i=1,self.num_state do table.insert(state, out[i]) end -- next_c, next_h
  end
...
function layer:sample_beam(imgs, opt)
  ...
  for k=1,batch_size do
    ...
    table.insert(all_result, done_beams)
  ...
  -- return the samples and their log likelihoods
  io.write('B-')
  return seq, seqLogprobs, all_result
end
...
function layer:updateOutput(input)
  ...
  local seqLogprobs = torch.FloatTensor(self.seq_length, batch_size)
	local logprobs
	local sampled = torch.Tensor(self.seq_length):zero()
	local inputW  = torch.Tensor(self.seq_length):zero()
	local cap     = torch.Tensor(self.seq_length):zero()

  for t=1,self.seq_length+2 do
    local can_skip = false
    local xt, sampleLogprobs
    -- setting the word(img) input and previous hidden state
    if t == 1 then
      -- feed in the images
    ...
    elseif t == 2 then
      ...
      --[[
        ...
        token START/END will do, arbitrarily. This will be ignored anyway
      ...
      it[torch.eq(it,0)] = self.vocab_size+1
			...
			-- sampling between gt word and sampled word
      local sampleLogprobs, predicted = torch.max(logprobs, 2)
      sampled[t-2] = predicted[1]:long()
			if self.scheduled_sampling >= 0 then
        local s_prob = torch.Tensor{self.eta, 1-self.eta}
				local sampling = torch.multinomial(s_prob, 1, true)
				--print('eta : ', self.eta, '   sampling : ', sampling[1])
				if sampling[1] == 2 then
					--print('------predicted word is used as input')
					it = predicted:view(-1):long()
          inputW[t-2] = it[1]
				end
			end
    ...
    if not can_skip then
      ...
      self.inputs[t] = {xt,unpack(self.state[t-1])} -- word, prev_c, prev_h
      -- forward the network
      local out = self.clones[t]:forward(self.inputs[t])
			logprobs = out[self.num_state+1]
      -- process the outputs
      self.output[t] = out[self.num_state+1] -- last element is the output vector, logprobs
	...
	io.write('caption : ');    self:print_caption(cap)
	io.write('\nsampled : ');  self:print_caption(sampled)
  if self.scheduled_sampling >= 0 then io.write('\ninput   : '); self:print_caption(inputW) end
	io.write('\n')
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    local dinputs = self.clones[t]:backward(self.inputs[t], dout) -- dinputs = dxt, dprev_c, dprev_h
...
--[[
N : # image (=batch size)
D : length of caption (sequence) 
input is a Tensor of size (D+2) x N x (M+1)
...
function crit:updateOutput(input, seq)
  ...
  local D = seq:size(1)
  assert(D == L-2, 'input Tensor should be 2 larger in time') -- added start and end token
  ...
  local loss = 0
    ...
    for t=2,L do -- iterate over sequence time (ignore t=1, dummy forward for the image)
      ...
      -- the first time we see null token as next index, actually want the model to predict the END token
			-- After End token, all gradient are zero
      if target_index == 0 and first_time then
        target_index = Mp1  -- END token
        first_time = false
      end
